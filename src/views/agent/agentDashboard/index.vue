...
import { queryMediumDashboard } from "@/api/agent.js";
...
const initCharts = () => {
  ...
  lineChart.setOption({
    ...
    legend: {
      data: [
        t("dashboard.inOrder"),
        t("dashboard.outOrder"),
        t("dashboard.withdrawalOrder"),
      ],
      //   data: ["出金", "入金"],
    },
    xAxis: {
    ...
    series: [
      ...
      {
        name: t("dashboard.withdrawalOrder"),
        type: "line",
        data: data.value.yforWithdrawalOrder,
      },
    ],
  });
};
const getData = async () => {
  const response = await queryMediumDashboard();
